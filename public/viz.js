...
var options = {
	width : 700,
	height : 400,
	title : 'Session Hours Provided by University of Portland Librarians in a Given Year',
	hAxis : {
		title : 'Month',
		gridlines : {
			count : 12
...
function vizController(thisYear) {
	...
	//Determines whether the DataView object for a given year already exists
	if(views[thisYear]!=null){
		chart.draw(views[thisYear].toDataTable(), options);
	}
	//If view[thisYear] does not exist, creates DataView object to get the subset of data for the year
	else {

		// creates the object and get the rows
		// corresponding to "thisYear".
		views[thisYear] = new google.visualization.DataView(data);
		...
		column : 2,
		value : thisYear}]));
		//Renders the chart for DataView of view[thisYear]
